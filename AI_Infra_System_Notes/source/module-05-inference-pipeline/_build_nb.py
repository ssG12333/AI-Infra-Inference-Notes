"""Build module-05-notebook.ipynb — Phase 5 深度实战 + 全流程模拟"""
import json
...
def code(src, cid):
    nb['cells'].append({'cell_type': 'code', 'metadata': {}, 'id': cid,
                        'source': src, 'outputs': [], 'execution_count': None})

# ============================================================
# Title + Env
# ============================================================
md(["Module 5: LLM 推理流水线——一个 Token 的奇幻漂流\n\n",
    "> 从模板包装到 BPE 分词，从 RoPE 旋转到 Embedding 查表，从 28 层 Transformer 到 KV Cache 拼接，"\
    "从概率采样到字节解码——用代码追踪一个 token 的完整旅程。\n",
    ">\n",
    "> **本章重点配套第3章（8 Phase 全景），用可运行的 mini 组件验证每个 Phase。**\n"], 'm01')

md(["## 0. 环境准备\n"], 'm02')
md(["导入所有需要的库。`torch.nn` 构建 mini decoder，`numpy` 辅助计算，`matplotlib` 可视化。\n"], 'm03')
code([
    "import torch\n",
...
# --- 5.1 Build MiniDecoderLayer ---
md(["#### 5.1 构建 MiniDecoderLayer——与 Qwen3 同架构的缩小版\n\n",
...
# --- 5.2 Prefill vs Decode — Same Layer, Different Worlds ---
md(["#### 5.2 Prefill vs Decode——同样的 Layer，完全不同的世界\n\n",
...
# --- 5.3 KV Cache 拼接演示 ---
md(['#### 5.3 KV Cache 拼接——Decode 的「记忆追加」机制\n\n',
...
# --- 5.5 GQA Repeat ---
md(["#### 5.5 GQA (Grouped Query Attention) ——K/V 头数减半的秘密\n\n",
...
# ============================================================
# Phase 5.1: Decoder 内部结构 (ASCII art but clear)
# ============================================================
md(["---\n### Phase 5.1: Decoder 内部结构\n\n",
    "每层 = Pre-RMSNorm → Q/K/V 投影 → GQA Repeat → SDPA (KV Cache 出入口) → O 投影 + 残差 → ",
...
# ============================================================
# Phase 6-8: Projection + Sampling + Decode
# ============================================================
...
out_path = os.path.join(out_dir, 'module-05-notebook.ipynb')
