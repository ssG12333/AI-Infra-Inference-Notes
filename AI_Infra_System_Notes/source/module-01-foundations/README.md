...
1. **手算梯度**：Loss = (wx − 3)², w=0, x=2, lr=0.1。做 3 步 SGD 更新，观察 w 是否趋近 1.5（最优值）。

2. **广播判断**：`[3, 1, 4] + [1, 5, 4]` → 输出 shape？`[2, 3, 4] + [5, 4]` → 能广播吗？

3. **激活函数手绘**：在同一张图上画出 ReLU, GELU, Swish。标注 x=−3, 0, 3 三点的函数值。

4. **BN 融合推导**：Conv: W=[[2,1],[0,3]], b=[0.5,−0.2]; BN: γ=[2,1], β=[0,0], μ=[1,2], σ²=[4,1]。手算 W' 和 b'。

5. **Roofline 判断**：Qwen3 decode 阶段 Q 投影 (batch=1, [1,1024]×[1024,2048])，计算计算强度。Memory-bound 还是 compute-bound？
